    ...
    if(registroForm.valid){
      ...
      this._clienteService.registro_direccion_cliente(data,this.token).subscribe(
        response => {
         ...
         $("#sl-region").attr('disabled', true);
         $("#sl-provincia").attr('disabled', true);
         $("#sl-distrito").attr('disabled', true);
         ...
         iziToast.show({
          ...
          class: 'text-success',
          position: 'topRight',
          message: 'Tus agrego la nueva direccion correctamente'
        });
        }
      )
    }else{
